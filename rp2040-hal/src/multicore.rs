...
#[link_section = ".stack1"]
#[used]
static mut CORE1_STACK: [u8; 4096] = [0; 4096];
extern "C" {
...
impl<'p> Multicore<'p> {
    ...
    fn spawn(&mut self, id: usize, wrapper: *mut (), entry: *mut ()) {
        ...
        let core1_stack = unsafe { &mut CORE1_STACK } as *mut [u8; 4096] as *mut usize;

        let mut stack_ptr =
            unsafe { core1_stack.add(CORE1_STACK.len() / core::mem::size_of::<usize>()) };

        stack_ptr = unsafe { stack_ptr.sub(3) };

        unsafe {
            stack_ptr.write_volatile(entry as usize);
            stack_ptr.add(1).write_volatile(core1_stack as usize);
